...
class FileList extends React.Component {
  ...
  render() {
    ...
    let fileItems = fileList && fileList.map((file, index)=>{
      return (<li key={file.id} data-id={file.id} className={ (!selectId && index == 0) || selectId == file.id ? 'li-selected' : ''}>
                { reName && selectId == file.id ? (<input value={file.title} onChange={this.reTitle}/>) : file.title }
                { 
