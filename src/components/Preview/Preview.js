...
class Preview extends React.Component {
  ...
  getData = (hash) => {
    get(apiHost + '/api/ipfs/cat', { path: hash })
    .then((res)=>{
      console.log(res);
      ...
      <div className="wrap">
        <Header>
          <div className="person-hd-logo" onClick={this.gotoLogin}>
            <div className="person-hd-logo-ico"></div>
          </div>
        </Header>
        <div className="preview-wrap" dangerouslySetInnerHTML={{ __html:  `<h1>${blog.title || ''}</h1><p></p>${blog.content || ''}`}}>
        </div>
        {
          loading && 
          <Loading2 />
