    ...
    return (
      <div className="myinfo-wrap">
        <div className="myinfo-menu">
          <ul>
            <li className="ico-ank" onClick={this.props.hideMyInfo}></li>
            <li className="on">我的帐户</li>
          </ul>
        </div>
        <div className="myinfo-detail">
          <div className="hd" onClick={this.gotoHomepage}><a>我的主页</a></div>
          <div className="info-edit" onClick={edit}><a>编辑</a></div>
          <ul>
            <li>
              <p>昵称 { <i className="set-nickname" onClick={()=>{ this.setState({ showSetting: true }) }}>设置</i>}</p>
          ...
          <div className="exit" onClick={this.props.loginOut}>退出</div>
        </div>
          <Modal 
          visible={showSetting}
          confirm={this.setNickname}
          cancel={()=>{ this.setState({ showSetting: false }) }}
          >
          <div style={{ width: '350px' }}>
            <p style={{ padding: '10px' }}>请输入昵称<NickNameWarning>(只能设置一次,不能再修改)</NickNameWarning></p>
            <input type="text" className="lg-input" value={nickName} onChange={this.nickNameChange}/>
            <p style={{ padding: '10px' }}>请输入密码</p>
            <input type="password" className="lg-input" onChange={this.pwdChange}/>
          </div>
        </Modal>
      </div>
    );
